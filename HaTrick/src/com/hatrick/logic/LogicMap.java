...
public class LogicMap {
    ...
    public void damage(double pos_x, double pos_y, double range, double power) {
        ...
        for (int i = (int)Math.floor(up / ratio); i <= (int)Math.ceil(down / ratio); i ++)
            for (int j = (int)Math.floor(left / ratio); j <= (int)Math.ceil(right / ratio); j++) {
                LogicObject iter = objMap[i][j];
                while (iter != null) {
                	 /*TODO: if ((iter.pos_x - pos_x) * (iter.pos_x - pos_x)
